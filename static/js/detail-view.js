/* 個股詳細資料檢視 — 可重複使用模組
   同一份渲染邏輯同時供兩處使用：
     1. /stock/<code> 獨立頁面   → layout: "page"
     2. 首頁右側詳細資料面板     → layout: "panel"
   用法：var view = StockDetail(rootEl, {layout:"panel"}); view.load("2330");
*/
window.StockDetail = (function () {
  ...
  // ---------------------------------------------------------- 骨架 HTML
  function skeleton(isPanel) {
    var head =
      '<div class="dhead">' +
        '<div class="dhead-main">' +
          '<h1 data-el="title">載入中…</h1>' +
          '<div class="sub" data-el="subInfo">正在取得行情資料…</div>' +
        "</div>" +
        '<div class="spacer"></div>' +
        '<div class="dhead-px">' +
          '<div class="px" data-el="px">—</div>' +
          '<div class="chg" data-el="chg">—</div>' +
        "</div>" +
        '<div class="gradebox">' +
          '<div class="g" data-el="grade">—</div>' +
          '<div class="t" data-el="gradeTxt">綜合評分</div>' +
        "</div>" +
      "</div>";
    ...
    var chartCard =
      '<div class="card">' +
        "<h3>K 線圖" +
          '<span class="periods" data-el="periods">' +
            '<button data-p="3mo">3月</button>' +
            '<button data-p="6mo">6月</button>' +
            '<button data-p="1y" class="on">1年</button>' +
            '<button data-p="2y">2年</button>' +
          "</span>" +
        "</h3>" +
        '<div class="kchart" data-el="kchart"></div>' +
      "</div>";
      ...
      '<div class="card">' +
        ...
        '<div class="tablewrap">' +
          '<table class="mini"><thead><tr>' +
            '<th style="text-align:left">日期</th><th>開盤</th><th>最高</th><th>最低</th>' +
            "<th>收盤</th><th>漲跌幅</th><th>成交量(張)</th>" +
          '</tr></thead><tbody data-el="recent"></tbody></table>' +
        "</div>" +
      "</div>";

    var scoreCard =
      '<div class="card">' +
        '<h3>評分拆解 <span class="tag" data-el="totalTag"></span></h3>' +
        '<div class="bd" data-el="breakdown"></div>' +
      "</div>";

    var techCard = '<div class="card"><h3>技術指標</h3><div class="kv" data-el="tech"></div></div>';
    var fundCard = '<div class="card"><h3>基本面</h3><div class="kv" data-el="fund"></div></div>';

    if (isPanel) {
      // 面板為單欄堆疊，由上而下閱讀
      return head + '<div class="stack">' + chartCard + scoreCard + techCard + fundCard + recentCard + "</div>";
    }
    return head +
      '<div class="grid2">' +
        '<div class="stack">' + chartCard + recentCard + "</div>" +
        '<div class="stack">' + scoreCard + techCard + fundCard + "</div>" +
      "</div>";
  }
  ...
  function create(root, opts) {
    opts = opts || {};
    var isPanel = opts.layout === "panel";
    var chart = null, period = "1y", payload = null, code = null, reqSeq = 0;

    root.classList.add("detailview", isPanel ? "as-panel" : "as-page");
    root.innerHTML = skeleton(isPanel);
    ...
    function renderHead() {
      ...
      el.title.textContent = payload.stock.code + " " + payload.stock.name;
      el.px.textContent = fmt(q.close);
      el.px.className = "px " + cls(q.change);
      el.chg.textContent = sign(q.change) + "  (" + sign(q.change_pct, 2, "%") + ")";
      el.chg.className = "chg " + cls(q.change);
      ...
      el.grade.textContent = rt.grade;
      el.grade.className = "g " + cls(rt.total - 58);
      el.gradeTxt.textContent = rt.label + " · " + fmt(rt.total, 1) + " 分";
      if (!isPanel) {
    ...
    function renderBreakdown() {
      ...
      el.totalTag.textContent = "總分 " + fmt(rt.total, 1) + " / 100";
      el.breakdown.innerHTML = rt.breakdown.map(function (b) {
    ...
    function renderChart() {
      ...
      if (typeof echarts === "undefined") {
        el.kchart.innerHTML = '<div class="chartfail">圖表函式庫載入失敗</div>';
        return;
      }
      if (!chart) chart = echarts.init(el.kchart, null, { renderer: "canvas" });

      var volColors = c.candles.map(function (k) { return k[1] >= k[0] ? UP : DOWN; });
      var startPct = Math.max(0, 100 - 12000 / Math.max(c.dates.length, 1));
      var lp = isPanel ? 50 : 62;          // 面板較窄，左軸留白縮小
      var fsz = isPanel ? 10 : 11;

      var axisBase = {
        type: "category", data: c.dates, boundaryGap: true,
        axisLine: { lineStyle: { color: "#2a3442" } },
        axisLabel: { color: "#8a97a8", fontSize: fsz },
      ...
      chart.setOption({
        ...
        animation: false,
        legend: {
          top: 6, left: 12, textStyle: { color: "#8a97a8", fontSize: fsz },
          itemGap: isPanel ? 9 : 14, icon: "roundRect", itemWidth: 14, itemHeight: 3,
          data: isPanel
            ? ["K線", "MA5", "MA20", "MA60"]
            : ["K線", "MA5", "MA20", "MA60", "布林上軌", "布林下軌"]
        },
        tooltip: {
          ...
          formatter: function (ps) {
            ...
            var line = function (k2, v) {
              return '<div style="display:flex;gap:16px;justify-content:space-between">' +
                '<span style="color:#8a97a8">' + k2 + "</span><span>" + v + "</span></div>";
            };
        ...
        grid: [
          { left: lp, right: 22, top: 34, height: "44%" },
          { left: lp, right: 22, top: "56%", height: "10%" },
          { left: lp, right: 22, top: "69%", height: "11%" },
          { left: lp, right: 22, top: "83%", height: "11%" }
        ],
        xAxis: [
        ...
        yAxis: [
          ...
          Object.assign({}, yBase, {
            ...
            axisLabel: {
              color: "#8a97a8", fontSize: 10,
              formatter: function (v) { return Math.round(v / 1000) + "張"; }
            }
          }),
    ...
    // ------------------------------------------------------ 載入
    function load(nextCode) {
      if (nextCode) code = nextCode;
      if (!code) return;

      var seq = ++reqSeq;          // 連點多檔時只採用最後一次請求的結果
      root.classList.add("is-loading");
      el.subInfo.textContent = "載入中…";

      fetch("/api/stock/" + encodeURIComponent(code) + "?period=" + period)
        .then(function (r) { return r.json().then(function (d) { return { ok: r.ok, d: d }; }); })
        .then(function (res) {
          if (seq !== reqSeq) return;      // 已有更新的請求，捨棄這筆結果
          if (!res.ok) {
            el.subInfo.textContent = res.d.error || "載入失敗";
            el.kchart.innerHTML = '<div class="chartfail">' + (res.d.error || "無法取得行情資料") + "</div>";
            return;
          }
          payload = res.d;
          renderHead();
        ...
        .catch(function (e) {
          ...
          el.subInfo.textContent = "載入失敗：" + e;
        })
        .finally(function () {
          if (seq === reqSeq) root.classList.remove("is-loading");
        });
    }

    // ------------------------------------------------------ 事件
    el.periods.addEventListener("click", function (e) {
      var b = e.target.closest("button[data-p]");
      if (!b || b.classList.contains("on")) return;
      period = b.dataset.p;
      Array.prototype.forEach.call(el.periods.children, function (node) {
        node.classList.toggle("on", node === b);
      });
      load();
    });
